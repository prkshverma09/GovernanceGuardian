...
def test_customer_index_exists(es_client):
    """Verify that the customer-leads-prod index exists."""
    assert es_client.indices.exists(index="customer-leads-prod")

@pytest.mark.integration
def test_customer_mapping_correct(es_client):
    """Verify the mapping for customer-leads-prod."""
...
def test_customer_count(es_client):
    ...
    count = es_client.count(index="customer-leads-prod")["count"]
...
def test_esql_minors_query(es_client):
    ...
    query = 'FROM "customer-leads-prod" | WHERE Age < 18 | STATS count = COUNT(*) | KEEP count'
...
def test_esql_restricted_zone_query(es_client):
    ...
    query = 'FROM "customer-leads-prod" | WHERE Country == "GDPR_Restricted_Zone" | STATS count = COUNT(*) | KEEP count'
    res = es_client.esql.query(query=query)
    count = res["values"][0][0]
    assert count > 0
